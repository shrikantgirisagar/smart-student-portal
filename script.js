...
function updateLoginUsernameLabel(role) {
  ...
  if (forgotRow) {
    forgotRow.style.display = role === "admin" ? "none" : "";
  }
}
...
resetModal.className = "modal-backdrop hidden";
resetModal.innerHTML = `
  <div class="reset-modal">
    <button type="button" id="closeReset" class="close-modal" aria-label="Close">×</button>
    <div class="modal-icon">🔒</div>
    <div class="modal-title">
      <span id="resetRoleLabel">PASSWORD RESET</span>
      <h2 id="resetTitle">Forgot Password</h2>
      <p>Reset your password using your registered email address.</p>
    </div>
    <form id="resetForm">
      <div id="resetStepEmail">
        <label id="resetEmailLabel">Registered Email Address</label>
        <div class="input-wrap"><span class="input-icon">✉️</span><input id="resetEmail" type="email" required placeholder="Enter your registered email address" autocomplete="email"></div>
        <button class="primary-btn" type="button" id="sendOtpBtn"><span>Send OTP</span><span class="arrow">→</span></button>
        <div id="resetOtpNotice" class="reset-otp-notice hidden"></div>
      </div>
      <div id="resetStepOtp" class="hidden">
        <label>Enter OTP</label>
        <div class="input-wrap"><span class="input-icon">🔢</span><input id="resetOtp" type="text" inputmode="numeric" maxlength="6" placeholder="Enter OTP"></div>
        <button class="primary-btn" type="button" id="verifyOtpBtn"><span>Verify OTP</span><span class="arrow">→</span></button>
      </div>
      <div id="resetStepPassword" class="hidden">
        <label>New Password</label>
        <div class="input-wrap"><span class="input-icon">🔒</span><input id="resetNewPassword" type="password" minlength="6" placeholder="Enter new password"></div>
        <label>Re-enter Password</label>
        <div class="input-wrap"><span class="input-icon">🔒</span><input id="resetConfirmPassword" type="password" minlength="6" placeholder="Re-enter new password"></div>
        <button class="primary-btn" type="button" id="updatePasswordBtn"><span>Update Password</span><span class="arrow">→</span></button>
      </div>
...
function updateResetModalStep(step) {
  ...
  if ($("resetOtpNotice")) $("resetOtpNotice").classList.toggle("hidden", step !== 2);
}

function openResetModal() {
  if (currentRole === "admin") {
    $("loginMessage").textContent = "Password reset is only available for Student and Faculty accounts.";
...
async function handleVerifyOtp() {
  const email = $("resetEmail").value.trim().toLowerCase();
  const otpValue = $("resetOtp").value.trim();
  if (!/^\d{6}$/.test(otpValue)) {
    setResetMessage("Enter the 6-digit OTP.");
    return;
  }
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/reset/verify-otp`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ role: resetRole, email, otp: otpValue })
    });
    const data = await response.json();
...
async function handleUpdatePassword() {
  ...
  if (password.length < 6) {
    setResetMessage("Password must contain at least 6 characters.");
    return;
  }
  if (password !== confirm) {
